...
```sh
statement : statementList | ifStatement | whileStatement | doStatement | forStatement | syncStatement | tryStatement | bytecode
statement : throwStatement | branchStatement

statementList : '{' '}' | '{' statementListElement { semi statementListPart } '}'
statementListPart : label? ( expression | variable )
label : identifier ':'
variable : ( variableModifier | annotation )? type identifier '=' expression

ifStatement : 'if' '(' expression ')' expression ( semi? 'else' expression )?
...
branchStatement : 'break' identifier?
branchStatement : 'continue' identifier?
branchStatement : 'goto' identifier
...
```sh
instruction : simpleInstruction | varInstruction | intInstruction | multiArrayInstruction | fieldInstruction | methodInstruction | jumpInstruction
...
methodInstruction : identifier internalType '.' identifier '(' ( internalType { ',' internalType } )? ')' ':' internalType
